...
{
	CalculateTangents(vertArray, vertsInArray, indxArray, indxsInArray);
...
Mesh::Mesh(const char* fileName, Microsoft::WRL::ComPtr<ID3D11Device> deviceObj) 
{
	...
	while (obj.good())
	{
		...
		else if (chars[0] == 'v' && chars[1] == 't')	// uv
		{
		...
		else if (chars[0] == 'f')						// faces (indices)
		{
